...
least_cost_distance = gdistance::costDistance(tr_c, pc, pc)
...
# tr = gdistance::transition(world_nosea_nb, mean, directions=8)
# plot(raster(tr), xlim=c(-5, 5), ylim=c(50, 60))
# plot(tr)

# tr_c = gdistance::geoCorrection(tr, "c", scl=TRUE) # cost?
# tr_r = gdistance::geoCorrection(tr, "r", scl=TRUE) # resistance?
# plot(raster(tr_c), xlim=c(-5, 5), ylim=c(50, 60))
# plot(raster(tr_r), xlim=c(-5, 5), ylim=c(50, 60))

# plot(raster(tr_c))
# points(pc)
# least_cost_distance = gdistance::costDistance(tr_c, pc[1, ], pc[2, ])

# # Europe = raster::crop(World, extent(-30, 30, 30, 70))
# # Europe = aggregate(Europe, 10) # combine cells
# # europe_poly = raster::crop(wrld_simpl, extent(-30, 30, 30, 70))
# # europe_poly = rgeos::gUnaryUnion(europe_poly)
# # # sea = raster::crop(Europe, extent(-30, -20, 40, 50))
# # Europe_nosea = raster::mask(Europe, europe_poly)
# # Europe_nosea[!is.na(Europe_nosea)] = 1
# # plot(Europe_nosea)
...
# geo_dist = raster::pointDistance(pc, longlat=TRUE)
# geo_dist = as.dist(geo_dist)

# tr = gdistance::transition(Europe_nosea, mean, directions=8)
# plot(raster(tr))
...
# least_cost_distance = gdistance::costDistance(tr_c, pc)
# commute_distance = gdistance::commuteDistance(tr_r, pc)

# # Europe_nosea[is.na(Europe_nosea)] = 1e3
# Europe_nosea = reclassify(Europe_nosea, cbind(1e3, 0))
# tr = gdistance::transition(Europe_nosea, mean, directions=8)
# plot(raster(tr))
...
# plot(europe_poly)
# points(pc[1:2, ], col=2, pch='+')
# plot(gdistance::shortestPath(tr, pc[2, ], pc[1, ], output="SpatialLines"), col=2, add=T)
# plot(gdistance::shortestPath(tr, pc[2, ], pc[3, ], output="SpatialLines"), col=2, add=T)
# plot(gdistance::shortestPath(tr, pc[3, ], pc[2, ], output="SpatialLines"), col=2, add=T)
# plot(gdistance::shortestPath(tr, pc[4, ], pc[1, ], output="SpatialLines"), col=2, add=T)
# plot(gdistance::shortestPath(tr, pc[3, ], pc[1, ], output="SpatialLines"), col=2, add=T)
# plot(gdistance::shortestPath(tr, pc[2, ], pc[1, ], output="SpatialLines"), col=2, add=T)
